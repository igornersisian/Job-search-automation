...
def run_actor(keywords: list[str]) -> tuple[str, str]:
    ...
    # ATS actor caps queries at 5 (HTTP 400 'Input is not valid: Field input.queries
    # must NOT have more than 5 items' on anything larger).
    actor_input = {
        "queries": keywords[:5],
        "is_remote": True,
        "posted_after": since,
    ...
    apify_client.raise_for_status_verbose(resp, f"ATS start (input={json.dumps(actor_input, ensure_ascii=False)[:500]})")
...
def wait_for_run(run_id: str, token: str, timeout_seconds: int = 600) -> str:
    ...
    while time.time() < deadline:
        ...
        apify_client.raise_for_status_verbose(resp, f"ATS poll run={run_id}")
        data = resp.json()["data"]
        status = data["status"]
...
def fetch_dataset(dataset_id: str, token: str) -> list[dict]:
    ...
    apify_client.raise_for_status_verbose(resp, f"ATS dataset={dataset_id}")
